...
export class GameDetailsComponent extends UnsubscribeAbstract {
  ...
  user$: Observable<IUser | undefined> = this.authHttpService.user$.pipe(
    shareReplay()
  );
        ...
        switchMap((user: IUser | undefined) => {
          ...
          if (
            ...
            (params['creator'] === user?.username ||
              params['master'] === user?.username)
          ) {
            //check master from query for master or default request
  ...
  private updateMeta(item: IGameResponse) {
    const url = item.creator
      ? `${environment.url}/games/game-request/${item.creator.username}/${item._id}`
      : `${environment.url}/games/${item.master.username}/${item._id}`;
    this.metaHelper.updateMeta({
      ...
      url: url,
      imgUrl:
        item.imgUrl || 'https://eneri.com.ua/assets/images/eneri-social.jpg',
